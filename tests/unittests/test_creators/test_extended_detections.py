...
from depthai_nodes.message.creators import (
    create_extended_detection_message,
)
...
def test_valid_input():
    message = create_extended_detection_message(
        bboxes=BBOXES,
        scores=SCORES,
        labels=LABELS,
        angles=ANGLES,
...
def test_valid_input_no_optional():
    message = create_extended_detection_message(BBOXES, SCORES)
...
def test_bboxes_scores_keypoints():
    message = create_extended_detection_message(
        bboxes=BBOXES, scores=SCORES, keypoints=KPTS
    )
...
def test_bboxes_masks():
    message = create_extended_detection_message(
        bboxes=BBOXES, scores=SCORES, masks=MASKS
    )
...
def test_no_masks_no_bboxes():
    ...
    message = create_extended_detection_message(
        bboxes=bboxes, scores=scores, masks=mask
    )
...
def test_invalid_bboxes_type():
    with pytest.raises(ValueError):
        create_extended_detection_message(BBOXES.tolist(), SCORES)


def test_invalid_bboxes_shape():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX[0], ONE_SCORE)


def test_invalid_scores_type():
    with pytest.raises(ValueError):
        create_extended_detection_message(BBOXES, SCORES.tolist())


def test_invalid_scores_shape():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX, np.array([ONE_SCORE]))


def test_mismatched_bboxes_scores_length():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX, np.array([0.9, 0.8]))


def test_invalid_labels_type():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX, ONE_SCORE, labels=[1])


def test_mismatched_bboxes_labels_length():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX, ONE_SCORE, labels=np.array([1, 2]))


def test_invalid_angles_type():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX, ONE_SCORE, angles=[45])
...
def test_invalid_angles_range():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX, ONE_SCORE, angles=np.array([400]))


def test_invalid_keypoints_type():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX, ONE_SCORE, keypoints=[[[0.1, 0.1]]])
...
def test_invalid_masks_type():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX, ONE_SCORE, masks=[[0, 1]])


def test_invalid_masks_shape():
    with pytest.raises(ValueError):
        create_extended_detection_message(ONE_BBOX, ONE_SCORE, masks=np.array([0, 1]))
