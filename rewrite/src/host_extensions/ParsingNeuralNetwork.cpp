...
std::shared_ptr<ParsingNeuralNetwork> ParsingNeuralNetwork::build(const std::shared_ptr<Camera>& input, const NNArchive& nnArchive,
                                                                  const std::optional<float> fps) {
...
std::vector<HostOrDeviceParser> ParsingNeuralNetwork::getParserNodes(const NNArchive& nnArchive) {
    ...
    for(std::size_t idx = 0; idx < newParsers.size(); ++idx) {
        ...
        if(const auto& entry = newParsers[idx]; std::holds_alternative<std::shared_ptr<BaseParser>>(entry)) {
            ...
            const auto parser = std::get<std::shared_ptr<DetectionParser>>(entry);
            nn->out.link(parser->input);
            parser->out.link(inputs[std::to_string(idx)]);
        } else {
