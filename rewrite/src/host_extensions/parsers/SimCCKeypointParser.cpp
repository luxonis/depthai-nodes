...
void SimCCKeypointParser::foggyGuessesForOneDim(const nn_archive::v1::Head& head,
    ...
    if(auto shape = keypointsOutputs[0].shape) {
        ...
        const int expected = nKeypoints * valuesPerKeypoint * (pixelSubdivisions * (inputImgDim * 2));
        ...
        for(auto& dim : *shape) {
            if(dim == nKeypoints * valuesPerKeypoint) {
                collapsedDimsAreInterleaved = head.metadata.extraParams.value("collapsed_dims_are_interleaved", false);
                spdlog::trace("Observed output dim {} == {} * {}, assuming collapsed dim", dim, nKeypoints, valuesPerKeypoint);
                break;
            }
        }
    }
...
void SimCCKeypointParser::inferConfigFromMultipleOutputs(const nn_archive::v1::Head& head,
    ...
    if(anyMissingLayouts) {
        ...
        std::vector<char> outputDims = {'X', 'Y', 'Z'};
        for(int i = 0; i < keypointsOutputs.size(); i++) {
...
void SimCCKeypointParser::buildImpl(const nn_archive::v1::Head& head, const nn_archive::v1::Model& model) {
    ...
    try {
        ...
        spdlog::warn("{}: pixel_subdivisions not found in metadata, using default value {}", NAME, pixelSubdivisions);
    }

    // Find the image input so we can educate our guesses
    auto imgInput = std::ranges::find_if(model.inputs, [](const auto& i) { return i.inputType == nn_archive::v1::InputType::IMAGE; });
    ...
    // TODO don't make assumptions that last dim of output(s) is/are SimCC
}

void SimCCKeypointParser::run() {
    while(isRunning()) {
        std::shared_ptr<Keypoints3D> outputMessage;
        // Build a new nested scope to prevent use after move because I wish I was in rust
        {
            ...
            outputMessage = std::make_shared<Keypoints3D>(std::move(result), std::move(output));
        }
