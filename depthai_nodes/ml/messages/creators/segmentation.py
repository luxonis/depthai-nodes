...
def create_segmentation_message(mask: np.ndarray) -> SegmentationMask:
    ...
    @param mask: Segmentation map array of shape (H, W) where each value represents a segmented object class.
    @type mask: np.array
    ...
    if not np.issubdtype(mask.dtype, np.integer):
        raise ValueError(f"Unexpected mask type. Expected an array of integers, got {mask.dtype}.")
