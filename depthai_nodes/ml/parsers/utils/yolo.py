...
def non_max_suppression(
    ...
    for img_idx, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            x = np.concatenate((box, conf, class_idx, other), 1)[conf.flatten() > conf_thres]
...
def parse_yolo_outputs(outputs: list, strides: list, anchors: np.ndarray, kpts = None) -> np.ndarray:
    output = None
