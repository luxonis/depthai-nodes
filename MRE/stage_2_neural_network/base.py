...
from helpers import DetectionsFromGatheredData
...
with dai.Pipeline(device) as pipeline:
    ...
    stage_2_face_detection = pipeline.create(Stage2NeuralNetwork).build(
        image_input=rgb_out,
        detections_input=people_detection.out,
        nn_source=FACE_DETECTION_MODEL,
        input_resize_mode=dai.ImageManipConfig.ResizeMode.LETTERBOX,
        fps=FPS,
    )
    face_detections = pipeline.create(DetectionsFromGatheredData).build(input=stage_2_face_detection.out)
    visualizer.addTopic(topicName="rgb", output=encoder.out)
    visualizer.addTopic(topicName="detections", output=people_detection.out)
    visualizer.addTopic(topicName="faces", output=face_detections.out)
